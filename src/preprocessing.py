...
def join_categories(data, config):
    ...
    if config['label'] in data.columns.to_list():

        # Rename SEDANG to TIDAK BAIK
        data['category'] = data['category'].replace('SEDANG', 'TIDAK BAIK')
...
def main():
    ...
    PATH_ENCODER_STASIUN = config['path_encoder_stasiun']
    ...
    # 8. Scale the data
    PATH_SCALER = config['path_scaler']
    ...
    # 9. Encode the label
    PATH_LABEL_ENCODER = config['path_encoder_label']
